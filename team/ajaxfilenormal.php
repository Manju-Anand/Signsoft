...
if($request == 'addEvent'){
	...
	if(!empty($title) && !empty($description) && !empty($start_date) && !empty($end_date) ){
		...
		// Insert record
		$sql = "INSERT INTO nomevents(title,description,start_date,end_date,empid) VALUES('".$title."','".$description."','".$start_date."','".$end_date."','" . $_SESSION['empid'] . "')";
...
if($request == 'moveEvent'){
	...
	if($eventid > 0 && !empty($start_date) && !empty($end_date) ){
		...
		// Check event id
		$sql = "SELECT id FROM nomevents WHERE id=".$eventid;
		$result = mysqli_query($connection,$sql);
		if(mysqli_num_rows($result)){
			// Update record
			$sql = "UPDATE nomevents SET start_date='".$start_date."',end_date='".$end_date."',empid='" . $_SESSION['empid'] . "' WHERE id=".$eventid;
			if(mysqli_query($connection,$sql)){
				$status = 1;
...
if($request == 'editEvent'){
	...
	if($eventid > 0 && !empty($title) && !empty($description)){
		...
		if(mysqli_num_rows($result)){
			...
			// Update record
			$sql = "UPDATE nomevents SET title='".$title."', description='".$description."',empid='" . $_SESSION['empid'] . "' WHERE id=".$eventid;
			if(mysqli_query($connection,$sql)){
...
if($request == 'deleteEvent'){
	...
	if($eventid > 0){
		...
		if(mysqli_num_rows($result)){
			...
			// Delete record
			$sql = "DELETE FROM nomevents WHERE id=".$eventid;
			if(mysqli_query($connection,$sql)){
				$status = 1;
