...
<body class="app sidebar-mini">
    ...
    if ($categoriesResult->num_rows > 0) {
        ...
        while ($categoryRow = $categoriesResult->fetch_assoc()) {
            // Check if the category is present in the order_category table
            $categoryId = $categoryRow['id'];
            $orderId = $_GET["edit"];
            $isCheckedCategory = isCategorySelected($orderId, $categoryId); // Implement this function
if ( $isCheckedCategory =="true"){

            echo '<li><input type="checkbox" name="categories[]" value="' . $categoryId . '" checked>' . $categoryRow['category'] . '</li>';
}else {

    echo '<li><input type="checkbox" name="categories[]" value="' . $categoryId . '">' . $categoryRow['category'] . '</li>';
}
                                ...
                                xhr.onreadystatechange = function () {
                                    if (this.readyState === 4 && this.status === 200) {
                                        ...
                                        for (var j = 0; j < subcategories.length; j++) {
                                            ...
                                            // // Check if the subcategory is present in the order_subcategory table
                                            // var isCheckedSubcategory = <?php echo json_encode(isSubcategorySelected($orderId, subcategories[j].id)); ?>;
                                            // subcategoryCheckbox.checked = isCheckedSubcategory;
                                            // Check if the subcategory is present in the order_subcategory table
                                            var subcategoryId = subcategories[j].id;
                                            
                                            var xhr = new XMLHttpRequest();
                                            xhr.onreadystatechange = function () {
                                                if (this.readyState === 4 && this.status === 200) {
                                                    var isCheckedSubcategory = JSON.parse(this.responseText);
                                                    subcategoryCheckbox.checked = isCheckedSubcategory;
                                                }
                                            };
                                            xhr.open("GET", "check_subcategory.php?order_id=" +  orgorderid + "&subcategory_id=" + subcategoryId, true);
                                            xhr.send();
    ...
    // Function to check if a category is selected for the given order
  
  

    // Function to check if a category is selected
// function isSelected($orderId, $categoryId, $table)
// {
//     global $connection;
//     $query = "SELECT COUNT(*) as count FROM $table WHERE order_id = $orderId AND category_id = $categoryId";
//     $result = $connection->query($query);
//     $row = $result->fetch_assoc();
//     return $row['count'] > 0;
// }
// Function to check if a category is selected
// function isSubcategoryedSelected($orderId, $subcategoryId, $table)
// {
//     global $connection;
//     $query = "SELECT COUNT(*) as count FROM $table WHERE order_id = $orderId AND category_id = $subcategoryId";
//     $result = $connection->query($query);
//     $row = $result->fetch_assoc();
//     return $row['count'] > 0;
// }
    ...
    ?>
                                                ...
                                                <div class="row mb-4">
                                                    ...
                                                    <div class="col-md-9">
                                                        <textarea class="form-control" name="statusreason" id="statusreason" palceholder="Here statusreason" rows="4"><?php echo $roworders['status_reason'] ?></textarea>
